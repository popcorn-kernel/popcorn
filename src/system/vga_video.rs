...
pub fn blit_buffer(buffer: &VGABuffer, _x: usize, _y: usize) {
    ...
    for yy in 0..buffer.h {
        for xx in 0..buffer.w {
            unsafe {
                ...
                frame_buffer
                    .add(offset)
                    .write_volatile(buffer.buffer[xx + yy * buffer.w]);
            }
        }
    }
}
...
impl VGABuffer {
    ...
    pub fn blit_buffer(&mut self, x: usize, y: usize, other: &VGABuffer) {
        ...
        for row in 0..(max_y - y) {
            ...
            self.buffer[self_row + x..max_x * self.w]
                .copy_from_slice(&other.buffer[other_row..(max_x - x) * self.h]);
        }
    }
}
